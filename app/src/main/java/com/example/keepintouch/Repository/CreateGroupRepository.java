...
public class CreateGroupRepository {
    ...
    {    String Date = new SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(new Date());
       ...
       String  AdminEmail=mFirebaseAuth.getCurrentUser().getEmail();
        ...
        mlist.add(AdminId);
        mFirebaseFirestore.collection("Zone").document(GroupId).set(new Zone(GroupId,GroupName,Code,"0","0","0",mlist,AdminId));
        mFirebaseFirestore.collection("Zone").document(GroupId).collection("memberList").document(AdminId).set(new MyLocation(AdminId,new Location(""),true));
